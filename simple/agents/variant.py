...
"""Currently, this is Conservative Devastate with Save City thing."""
# What if we just made it go to where the *enemy unit* is closest to? Fuckin just dick move it
...
def agent(observation, configuration):
	...
	def get_endangered_cities(cities):
		"""
		Returns a List of City objects that don't have enough fuel to survive the night
		cities: List of Cities (i.e player.cities)
		
		Returns 'closest_city_tile': List of Cities 
		"""
		endangered_cities: list[City] = []
		for k, city in cities.items():
			if (city.fuel < city.get_light_upkeep() * GAME_CONSTANTS["PARAMETERS"]["NIGHT_LENGTH"]):
				endangered_cities.append(city)			
			
		return endangered_cities

	def get_closest_endangered_cities(pos, city):
		"""
		Returns the closest citytile of an endangered city
		pos: Position Object
		city: City object
		
		Returns 'closest_city_tile': CityTile Object 
		"""
		closest_endangered_citytile = None
		if city.fuel < city.get_light_upkeep() * GAME_CONSTANTS["PARAMETERS"]["NIGHT_LENGTH"]: # This is for endangered cities
			# Find closest citytile from this city
			closest_dist = math.inf
			for citytile in city.citytiles:
				dist = citytile.pos.distance_to(pos)
				if dist < closest_dist:
					closest_dist = dist
					closest_endangered_citytile = citytile
		return closest_endangered_citytile
	...
	if game_state.turn == 0:
		print("I am devastator variant agent!!", file=sys.stderr)
...
#     print("Num citytile or units", num_citytiles, num_units, file=sys.stderr)
	...
	for k, city in player.cities.items():
		...
		# See if there's a big city that needs saving. This only saves one city though... And not always the biggest.
		if (city.fuel < city.get_light_upkeep() * GAME_CONSTANTS["PARAMETERS"]["NIGHT_LENGTH"]) and is_night and player.city_tile_count > opponent.city_tile_count and game_state.turn > 250:
			save_city = True
			endangered_city = city # Saves this variable so we can do stuff to it later
			print("I think we should save a city", file=sys.stderr)
	...
	for unit in player.units:
		...
		Place a city if possible
		Or else find an empty cell and place a city if your inventory is full
		...
		if unit.is_worker() and unit.can_act():
			print(str(player.units), file=sys.stderr)

			#Check if there's a big city with not enough fuel. If so & you have resources, go save it.
			#Save city!
			if save_city and unit.get_cargo_space_left() == 0:
				print("We're gonna save this endangered city!", endangered_city.cityid, file=sys.stderr)
				print(type(endangered_city), str(endangered_city.citytiles), file=sys.stderr)
				closest_endangered_citytile = get_closest_endangered_cities(unit.pos, endangered_city)
				move_unit(unit, closest_endangered_citytile.pos)
			...
			# We're full but unbuildable rn, find the closest place to build to. THis might go horribly wroong lol
				...
				if unit.get_cargo_space_left() > 0:
					...
					if closest_resource_tile is not None:
						...
						print("Well.. Shit. I'm unit,", unit.id ,"Apparently I can't find any resources.", file=sys.stderr)
